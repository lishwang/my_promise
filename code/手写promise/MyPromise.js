...
class MyPromise {
  ...
  //resolve 方法，不属于实例对象(在原型上看不到，在实例中也看不到)，只属于当前类，因此需要用 static 关键字进行描述，表明它是一个静态成员（不属于实例对象，只属于当前类）
  ...
  // reject 方法,不属于实例对象，只属于当前类,始终返回失败的promise
